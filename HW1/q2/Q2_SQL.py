...
class HW2_sql():
    ...
    def part_aiii(self,connection):
        ...
        part_aiii_sql = """CREATE TABLE IF NOT EXISTS cast_bio (
                                        ...
                                        birthday real,
                                        popularity real,
                                        cast_id integer ,FOREIGN KEY(cast_id) REFERENCES movie_cast(cast_id),
                                        
                                        
                                        CONSTRAINT castbio_PK PRIMARY KEY  (cast_id)
                                    );"""
        ######################################################################
    ...
    def part_c(self,connection):
        ...
        part_c_sql = """
            ...
            Select  printf("%.2f",(Count(title)* 100.0 / (Select Count(title) From movies))) from movies
    ...
    def part_d(self,connection):
        ...
        part_d_sql =  """
             ...
             where popularity > 10
             group by cast_name
             order by count(cast_name)  desc, cast_name  asc
             limit 5
                    """
        ######################################################################
    ...
    def part_e(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_e_sql = """
            SELECT m.title, printf("%.2f",m.score),count(mc.cast_id)
            FROM movies m
            inner join  movie_cast mc
            on  m.id= mc.movie_id 
            group by m.id
            order by score DESC, m.title ASC, count(distinct mc.cast_id) DESC
            limit 5
                    """
        ######################################################################
    ...
    def part_f(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_f_sql = part_f_sql = """
            SELECT mc.cast_id, mc.cast_name, printf("%.2f",avg(m.score)) 
            FROM movies m
            inner join  movie_cast mc
            on  mc.movie_id =m.id
            where  m.score >= 25
            ...
            order by printf("%.2f",avg(m.score))  DESC, mc.cast_name ASC
            limit 10
                    """
        ######################################################################
  ...
  select distinct c.cast_member_id1 as cast_member_id1 , c.cast_member_id2 as cast_member_id2, count(movie_id) as movie_count,avg(d.score) as average_movie_score from
  (
  select a.cast_id as cast_member_id1 ,b.cast_id as cast_member_id2 ,a.movie_id from movie_cast a inner join movie_cast b
  ...
  movies d
  on c.movie_id = d.id
  group by c.cast_member_id1 , c.cast_member_id2 having movie_count >=3 and average_movie_score >=40
"""
        ######################################################################
    ...
    def part_gi(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_g_i_sql = """select a.cast_id, b.cast_name, collaboration_score from 
 (select cast_member_id1 as cast_id,printf("%.2f",avg(average_movie_score)) as collaboration_score from good_collaboration group by cast_member_id1
union 
select cast_member_id2 as cast_id,printf("%.2f",avg(average_movie_score)) as collaboration_score from good_collaboration group by cast_member_id2) a
inner join 
(select cast_id,cast_name from movie_cast group by cast_id,cast_name) b
on a.cast_id = b.cast_id
order by collaboration_score desc, b.cast_name
limit 5"""
        ######################################################################
